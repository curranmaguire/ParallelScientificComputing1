...
class Test(object):
  ...
  def __init__(self, compiler, default_flags, submitted_file, benchmark_file = ""):
    """
  ...
  def compile(self,compiler_flags="-O3"):
    """
    ...
    success = True
    try:
      command="CXX=\"" + self.compiler + "\" CXXFLAGS=\"" + self.default_flags + " " + compiler_flags + "\" "
      invocation = command + "make " + self.__get_submitted_executable_name()
      print(invocation)
  ...
  def cleanall(self):
    subprocess.run("make -s cleanall", shell=True)
